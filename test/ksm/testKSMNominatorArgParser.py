...
class BounderTest(unittest.TestCase):
    ...
    def test_stop_nominate_tmp_without_args(self):
        stdIn, sdtOut = executeCliCommand(venv_env, main_script, "test_unnominate_tmp_without_args ", "ksm",
                                          "nominator",
                                          "stop_nominate_tmp")
        self.assertTrue(
            "error: the following arguments are required: -m/--mnemonic" in stdIn)
    ...
    def test_stop_nominate_tmp_success(self):
        stdIn, sdtOut = executeCliCommand(venv_env, main_script, "test_stop_nominate_tmp_success ", "ksm", "nominator",
    ...
    def test_stop_nominate_all_without_args(self):
        stdIn, sdtOut = executeCliCommand(venv_env, main_script, "test_stop_nominate_all_without_args", "ksm",
                                          "nominator",
                                          "stop_nominate_all")
        self.assertTrue(
            "error: the following arguments are required: -m/--mnemonic" in stdIn)
    ...
    def test_stop_nominate_all_success(self):
        stdIn, sdtOut = executeCliCommand(venv_env, main_script, "test_unnominate_all_success ", "ksm", "nominator",
